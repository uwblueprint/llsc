...
class CLI:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('output_format', choices=OUTPUT_FORMAT_CHOICES, help='Output format')
        parser.add_argument('--file_path', type=str, help='File path to save the data (required for csv, json, excel)', default=None)
        
        self.args = parser.parse_args()

    def validate_arguments(self):
        """Validate the parsed arguments."""
        if self.args.output_format in FILE_PATH_REQUIRED_FORMATS and not self.args.file_path:
            print(f"Error: file_path is required for formats: {', '.join(FILE_PATH_REQUIRED_FORMATS)}", file=sys.stderr)
            sys.exit(1)
    ...
    def run_seeder(self):
        ...
        if self.args.output_format == 'dataframe':
            ...
            seeder.save_data(output_format=self.args.output_format, file_path=self.args.file_path)
