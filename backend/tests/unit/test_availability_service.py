...
def db_session():
    ...
    try:
        ...
        session.execute(
            text(
                "TRUNCATE TABLE availability_templates, user_data, users RESTART IDENTITY CASCADE"
            )
        )
        session.commit()
...
async def test_pst_user_can_submit_8am_to_8pm(db_session, pst_volunteer):
    ...
    templates = db_session.query(AvailabilityTemplate).filter_by(
        user_id=pst_volunteer.id, is_active=True
    ).all()
