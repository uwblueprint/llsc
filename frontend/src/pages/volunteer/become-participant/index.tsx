...
export default function BecomeParticipantPage() {
  ...
  const advanceAfterUpdate = async (updated: IntakeFormData) => {
    ...
    if (nextType === 'thank-you') {
      setSubmitting(true);
      try {
        console.log('[BECOME_PARTICIPANT][SUBMIT] About to submit answers', {
          currentStep,
          nextType,
          answers: updated,
        });
        ...
        await router.replace('/volunteer/become-participant/thank-you');
        return;
      } finally {
        setSubmitting(false);
      }
