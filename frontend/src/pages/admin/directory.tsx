...
import { Box, Flex, Heading, Text, Table, IconButton, Input, Badge, Button, VStack } from '@chakra-ui/react';
import { FiSearch, FiMenu, FiMail, FiFolder, FiLoader } from 'react-icons/fi';
...
const getStatusColor = (step: string): { bg: string; color: string } => {
    ...
    if (lowerStep.includes('rejected')) return { bg: DIRECTORY_COLORS.statusRejectedBg, color: DIRECTORY_COLORS.statusRejectedText };
...
export default function Directory() {
    ...
    const handleClearFilters = () => {
        ...
        const clearedStatuses = { notStarted: false, inProgress: false, completed: false, rejected: false };
            ...
            <DirectoryDataProvider>
                {(users, loading, error) => {
                    const filteredUsers = users.filter((user: any) => {
                        ...
                        // User type filtering
                        const hasUserTypeFilter = appliedUserTypeFilters.participant || appliedUserTypeFilters.volunteer;
                        const matchesUserType = !hasUserTypeFilter ||
                            (appliedUserTypeFilters.participant && user.roleId === 1) ||
                            (appliedUserTypeFilters.volunteer && user.roleId === 2);

                        // Status filtering
                        const hasStatusFilter = appliedStatusFilters.notStarted || appliedStatusFilters.inProgress ||
                            appliedStatusFilters.completed || appliedStatusFilters.rejected;
                        const userStatus = user.formStatus && formStatusMap[user.formStatus as FormStatus]?.status;
                    ...
                    return (
                        <LightMode>
                            ...
                            {/* Main Content */}
                            <Box p={8} bg="white" minH="100vh" marginLeft={157} marginRight={130} paddingTop="124px">
                                <Heading
                                    as="h1"
                                    fontFamily="'Open Sans', sans-serif"
                                    fontWeight={600}
                                    ...
                                    <MenuRoot>
                                        ...
                                        <MenuContent p={4} minW="240px" borderRadius="8px" boxShadow="0px 8px 8px -4px rgba(10, 13, 18, 0.03), 0px 20px 24px -4px rgba(10, 13, 18, 0.08)">
                                            <VStack align="stretch" gap={4}>
                                                {/* User Type Section */}
                                                <Box>
                                                    <Text fontSize="14px" fontWeight="600" color={DIRECTORY_COLORS.navbarGray} mb={3}>
                                                        User type
                                                    </Text>
                                    ...
                                    >
                                        <Table.Header bg="white">
                                            <Table.Row bg="white" color="#535862" fontWeight="600" fontSize="16px">
                                                ...
                                                <Table.ColumnHeader width="12%" onClick={() => {
                                                    if (sortBy == 'nameDsc') {
                                                        setSortBy("nameAsc")
                                                    } else {
                                                        setSortBy('nameDsc')
                                            ...
                                            {filteredUsers.map((user: DirectoryUser) => {
                                                ...
                                                return (
                                                    <Table.Row key={user.id} bg={'white'} _hover={{ bg: 'gray.50' }}>
                                                        ...
                                                        <Table.Cell>
                                                            <Badge
                                                                bg={user.roleId === 2 ? DIRECTORY_COLORS.roleVolunteerBg : DIRECTORY_COLORS.roleParticipantBg}
                                                                color={user.roleId === 2 ? DIRECTORY_COLORS.roleVolunteerText : DIRECTORY_COLORS.roleParticipantText}
                                                                borderRadius="full"
                                                                px={3}
                                                                py={1}
                                                            >
                                                        ...
                                                        <Table.Cell>
                                                            {(() => {
                                                                const statusLabel = formStatusMap[user.formStatus]?.label || 'intake-submitted';
