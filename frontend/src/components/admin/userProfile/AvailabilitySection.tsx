  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={6}>
        <Heading color={COLORS.veniceBlue} fontWeight={600} fontSize="22px" lineHeight="1.82em">
          Availability
        </Heading>
        {isEditing ? (
          <HStack gap={2}>
      ...
      <Flex gap={8} align="flex-start">
        ...
        >
          ...
          >
            ...
            <GridItem p={2} borderBottom="1px solid" borderColor={COLORS.grayBorder} bg="gray.50">
              <Text fontSize="xs" color={COLORS.textSecondary} fontWeight="bold">
                EST
              </Text>
            </GridItem>
                ...
                <React.Fragment key={time}>
                  ...
                  {[0, 1, 2, 3, 4, 5, 6].map((dayIndex) => {
                    ...
                    if (isEditing) {
                      ...
                      isAvailable =
                        user.availability?.some((template) => {
                          ...
                          const startTimeIndex =
                            (startTime.hour - 8) * 2 + (startTime.minute === 30 ? 1 : 0);
                          const endTimeIndex =
                            (endTime.hour - 8) * 2 + (endTime.minute === 30 ? 1 : 0);
                    ...
                    const dragStartKey = dragStart
                      ? `${dragStart.dayIndex}-${dragStart.timeIndex}`
                      : '';
                    const willBeSelected =
                      isInDragRange && dragStartKey && !selectedTimeSlots.has(dragStartKey);
        ...
        {/* Summary Sidebar */}
        <Box w="200px">
          <Heading size="xs" mb={4} color={COLORS.veniceBlue}>
            Your Availability
          </Heading>
          <VStack align="stretch" gap={4}>
            {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(
              (day, index) => {
                ...
                const dayTemplates =
                  user.availability?.filter((template) => {
                ...
                return (
                  <Box key={day}>
                    <Text fontSize="xs" mb={1} color={COLORS.textPrimary}>
                      {day}:
                    </Text>
